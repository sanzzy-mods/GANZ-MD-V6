...
await conn.reply(m.chat, 'Uploading...', m, { contextInfo: {
  ...
  await conn.sendFile(m.chat, hakta.result, '[Silence BOT]' + ' ' + `${args[0]}` + '.pdf', '', m, false, { asDocument: true, thumbnail: thumbnail})
}
handler.help = ['doujin <code>']
handler.tags = ['nsfw']
handler.command = /^(nh|doujin)$/i
handler.limit = true
